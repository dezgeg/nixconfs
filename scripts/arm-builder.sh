...
if [ "$target" = channel ]; then
    ...
    if [ "$arch" = armv6 ]; then
        ...
        trace nix-build ./nixpkgs $nixopts -A ubootBananaPi -A ubootBeagleboneBlack -A ubootJetsonTK1 -A ubootPcduino3Nano -A ubootRaspberryPi2 -A ubootRaspberryPi3 -A ubootWandboard >> installer-closure
    fi
